					...
					<?php if ( version_compare( $porto_woo_version, '2.6', '>=' ) ) : ?>
						...
						<p class="woocommerce-form-row form-row clearfix">
							<?php if ( ( 'lost_password' ) === $args['form'] ) : ?>
								<a class="pt-left back-login text-v-dark" href="<?php echo esc_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ); ?>"><?php esc_html_e( 'Click here to login', 'woocommerce' ); ?></a>
							<?php endif; ?>
							<input type="hidden" name="wc_reset_password" value="true" />
							<button type="submit" class="woocommerce-Button button btn-lg pt-right btn-v-dark font-weight-normal text-capitalize" value="<?php esc_attr_e( 'Reset password', 'woocommerce' ); ?>"><?php esc_html_e( 'Reset password', 'woocommerce' ); ?></button>
						</p>
