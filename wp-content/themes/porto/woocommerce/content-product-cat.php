...
<li role="none" class="<?php echo esc_attr( trim( $class ) ); ?>">
	...
	?>
<?php if ( $whole_link ) : ?>
	<a href="<?php echo get_term_link( $category->slug, 'product_cat' ); ?>" aria-label="<?php esc_attr_e( 'Go to category page', 'porto' ); ?>">
<?php endif; ?>
...
<?php endif; ?>
	...
	<?php if ( 'category-pos-outside' == $view_type ) : ?>
		<a href="<?php echo get_term_link( $category->slug, 'product_cat' ); ?>" aria-label="<?php esc_attr_e( 'Go to category page', 'porto' ); ?>"><h4 class="m-t-md m-b-none"><?php echo esc_html( $category->name ); ?></h4></a>
		<?php
		if ( $category->count > 0 ) :
			$count_html = apply_filters( 'woocommerce_subcategory_count_html', ' <mark class="count">' . ( (int) $category->count ) . '</mark>', $category );
			if ( $count_html ) :
