	...
	if ( 'none' != $slideshow_type ) :
		...
		if ( 'images' == $slideshow_type ) :
			...
			<div class="row<?php echo 'widewidth' === $porto_layout ? ' m-n' : ''; ?>">
				<ul class="row portfolio-list<?php echo empty( $porto_settings['portfolio-zoom'] ) ? '' : ' lightbox'; ?>"<?php echo empty( $porto_settings['portfolio-zoom'] ) ? '' : ' data-plugin-options="' . esc_attr( $options ) . '"'; ?>>
					<?php
		...
		<div class="row">
			<div class="<?php echo 'wide-both-sidebar' == $porto_layout || 'both-sidebar' == $porto_layout ? 'col-md-12' : 'col-md-7'; ?> m-t-sm">
				...
				if ( get_the_content() ) :
					...
					<h4 class="portfolio-desc m-t-sm" aria-level="3"><?php printf( esc_html__( '%1$s %2$sDescription%3$s', 'porto' ), esc_html( $portfolio_name ), '<b>', '</b>' ); ?></h4><?php endif; ?>
