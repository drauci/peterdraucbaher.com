...
function porto_breadcrumbs() {
	...
	if ( function_exists( 'yoast_breadcrumb' ) && ! empty( $porto_settings['show-yoast-breadcrumbs'] ) ) {
	...
	if ( is_singular() ) {
		...
		if ( isset( $post->post_parent ) && 0 == $post->post_parent ) {
			...
			$ancestor_output = porto_breadcrumbs_ancestors_link();
			if ( $ancestor_output ) {
		...
		if ( is_page() || empty( $porto_settings['breadcrumbs-disable-single-leaf'] ) ) {
		...
		if ( is_post_type_archive() ) {
				...
				if ( ! empty( $arr ) ) {
					foreach ( $arr as $crumb ) {
						if ( $index == count( $arr ) - 1 ) {
							if ( empty( $porto_settings['breadcrumbs-disable-single-leaf'] ) ) {
								$output .= '<li>' . esc_html( $crumb[0] ) . '</li>';
							}
						} else {
			...
			} elseif ( class_exists( 'WCFMmp' ) && class_exists( 'WC_Breadcrumb' ) ) {
				$breadcrumbs = new WC_Breadcrumb();
				$arr         = apply_filters( 'woocommerce_get_breadcrumb', array(), $breadcrumbs );
				$index       = 0;
				if ( ! empty( $arr ) ) {
					foreach ( $arr as $crumb ) {
						if ( $index == count( $arr ) - 1 ) {
...
function porto_breadcrumbs_archive_name( $post_type ) {
	...
	if ( $page_id && function_exists( 'porto_multi_lang_post_id' ) ) {
		$page_id = porto_multi_lang_post_id( $page_id, 'page' );
	}
...
function porto_breadcrumbs_ancestors_link() {
	...
	foreach ( $post_ancestor_ids as $post_ancestor_id ) {
		...
		if ( ! $post_ancestor || is_wp_error( $post_ancestor ) ) {
			continue;
		}
