...
function porto_meta_layout() {
	...
	if ( ( class_exists( 'bbPress' ) && is_bbpress() ) || ( class_exists( 'BuddyPress' ) && is_buddypress() ) ) {
		...
		if ( ( function_exists( 'porto_is_shop' ) && porto_is_shop() && ! is_product_category() ) || $shop_builder ) {
			if ( $default ) {
				$value    = isset( $porto_settings['product-archive-layout'] ) ? $porto_settings['product-archive-layout'] : '';
		...
		}  elseif ( $acf_post_type = porto_has_acf_archive_layout() ) {
			$value    = $porto_settings[ $acf_post_type . '-acf-archive-layout' ];
			$sidebar  = isset( $porto_settings[ $acf_post_type . '-acf-archive-sidebar' ] ) ? $porto_settings[ $acf_post_type . '-acf-archive-sidebar' ] : '';
			$sidebar2 = isset( $porto_settings[ $acf_post_type . '-acf-archive-sidebar2' ] ) ? $porto_settings[ $acf_post_type . '-acf-archive-sidebar2' ] : '';
		} else {
		...
		if ( is_singular() ) {
			if ( $default ) {
				switch ( get_post_type() ) {
					...
					default:
						...
						} elseif ( $acf_post_type = porto_has_acf_single_layout() ) {
							$value    = $porto_settings[ $acf_post_type . '-acf-single-layout' ];
							$sidebar  = isset( $porto_settings[ $acf_post_type . '-acf-single-sidebar' ] ) ? $porto_settings[ $acf_post_type . '-acf-single-sidebar' ] : '';
							$sidebar2 = isset( $porto_settings[ $acf_post_type . '-acf-single-sidebar2' ] ) ? $porto_settings[ $acf_post_type . '-acf-single-sidebar2' ] : '';
						} else {
							$value    = $porto_settings['layout'];
							$sidebar  = $porto_settings['sidebar'];
							$sidebar2 = '';
			...
			if ( is_home() ) {
				$blog_id = get_option( 'page_for_posts' );
				if ( $blog_id && 'default' == get_post_meta( $blog_id, 'default', true ) ) {
					$value    = get_post_meta( $blog_id, 'layout', true );
					$sidebar  = get_post_meta( $blog_id, 'sidebar', true );
					$sidebar2 = get_post_meta( $blog_id, 'sidebar2', true );
				} else {
					$value = $porto_settings['post-archive-layout'];
				}
