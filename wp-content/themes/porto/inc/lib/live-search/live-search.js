...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.searchform' ).each( function () {
		...
		$this.find( 'input[type="text"]' ).on( 'focus keyup', function() {
			...
			if ( $input.hasClass( 'porto-search-init' ) || $input.closest( '#side-nav-panel' ).length > 0 ) {
				return;
			}
			...
			onSearchComplete: function () {
				$this.find( 'button' ).removeClass( 'loading' ).removeClass( 'porto-ajax-loading' ).find( '.porto-loading-icon' );
