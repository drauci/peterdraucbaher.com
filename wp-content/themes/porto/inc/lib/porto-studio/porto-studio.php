...
if ( ! class_exists( 'Porto_Studio' ) ) :
	...
	class Porto_Studio {
			...
			if ( 'post.php' == $GLOBALS['pagenow'] || 'post-new.php' == $GLOBALS['pagenow'] ) {
				if ( porto_is_elementor_preview() ) {
					...
					if ( defined( 'WPB_VC_VERSION' ) ) {
						...
						add_filter( 'vc_nav_front_controls', array( $this, 'add_studio_control' ), 5 );
						add_filter( 'vc_nav_controls', array( $this, 'add_edit_area_control' ) );
						add_filter( 'vc_nav_front_controls', array( $this, 'add_edit_area_control' ), 5 );
					}
					add_action( 'admin_enqueue_scripts', array( $this, 'enqueue' ), 1001 );
		...
		public function add_edit_area_control( $list ) {
			/**
			 * Fix dulplicated nav bar items issue
			 * 
			 * @since 7.4.3
			 */
		...
		public function filter_category() {
			...
			// Get Elementor blocks without Container Block
			$without_con = 'e' == $this->page_type && defined( 'ELEMENTOR_VERSION' ) && ! Elementor\Plugin::$instance->experiments->is_feature_active( 'container' );
			if ( $without_con ) {
				$blocks_without_con = get_site_transient( $transient_key . '_without_con' );
				if ( $blocks_without_con ) {
					$blocks = $blocks_without_con;
				}
			}
			if ( ! $blocks ) {
				require_once PORTO_PLUGINS . '/importer/importer-api.php';
				...
				if ( $without_con ) {
					...
					set_site_transient( $transient_key . '_without_con', $blocks, $this->update_period );
				}
			}
				...
				if ( isset( $posts[$old_id]['post_type'] ) && 'porto_builder' == $posts[$old_id]['post_type'] ) {	
					...
					if ( $post_id && ! is_wp_error( $post_id ) && !empty( $metas['porto_builder_type'] ) ) {
						...
						if ( false !== strpos( $posts[$old_id]['content'], '<!-- wp:porto' ) && class_exists( 'PortoBuildersType' ) && class_exists( 'PortoShortcodesClass' ) ) { // Gutenberg editor
							...
							$gutenberg_post = get_post( $post_id );
							if ( $gutenberg_post && ! is_wp_error( $gutenberg_post ) ) {
								$gutenberg_post->post_content = str_replace( 'u002d', '\u002d', $gutenberg_post->post_content );
								wp_update_post( $gutenberg_post );
								$tb_ins->save_meta_values( (int) $post_id, $gutenberg_post );
								do_action( 'porto_trigger_generate_post_css', (int) $post_id, $gutenberg_post );
							}
						}
					}
					unset( $posts[$old_id] );
			...
			if ( ! $block_categories ) {
				...
				if ( is_wp_error( $block_categories ) || ! $block_categories ) {
					...
					porto_get_template_part(
						...
						null,
						array(
							'error_content'    => ! empty( $invalid_response ) ? $invalid_response : sprintf( esc_html__( 'Please purchase or %1$sactivate%2$s Porto Theme.', 'porto' ), '<a target="_blank" href="' . esc_url( admin_url( 'admin.php?page=porto' ) ) . '">', '</a>' ),
						)
					);
		...
		public function render_frontend_block( $flag, $option, $default ) {
			if ( isset( $_POST['meta'] ) ) {
				$GLOBALS['porto_studio_meta'] = porto_filter_output( $_POST['meta'] );
		...
		public function update_custom_meta_fields_in_fronteditor() {
			...
			if ( isset( $_POST['fields'] ) && $_POST['post_id'] ) {
				...
				foreach ( $_POST['fields'] as $key => $value ) {
					...
					if ( '' != $original_value && strpos( $original_value, $value ) === false ) {
