...
if ( class_exists( 'WP_Importer' ) ) {
	class Porto_WP_Import extends WP_Importer {
			...
			switch ( $process ) {
				case 'import_start':
					...
					if ( ! in_array( $demo, $extra_demos ) && function_exists( 'wc_get_attribute_taxonomies' ) ) {
						$attribute_names  = array( 'Color', 'Size', 'Make', 'Model', 'Product Year' );
						// if ( 'shop53' == $demo || 'elementor-shop53' == $demo ) {
						// 	unset( $attribute_names[2] );
						// }
						if ( 'shop48' == $demo || 'elementor-shop48' == $demo ) {
						...
						} else if ( 'shop49' == $demo || 'elementor-shop49' == $demo ) {
							$attribute_names = array_merge(
								$attribute_names,
								array(
									'Arm Height',
						...
						foreach ( $attribute_names as $attribute_name ) {
							...
							if ( ! in_array( $attribute_name, $registered_attrs ) ) {
								...
								if ( 'Arm Height' === $attribute_name ) {
									$attribute['attribute_name'] = 'arm-height';
								} elseif ( 'Frame' === $attribute_name ) {
									$attribute['attribute_name'] = 'frame';
								} elseif ( 'Interior Seat Width' === $attribute_name ) {
									$attribute['attribute_name'] = 'interior-seat-width';
								} elseif ( 'Leg Height' === $attribute_name ) {
									$attribute['attribute_name'] = 'leg-height';
								} elseif ( 'Overall Dimensions' === $attribute_name ) {
									$attribute['attribute_name'] = 'overall-dimensions';
								} elseif ( 'Product Weight' === $attribute_name ) {
									$attribute['attribute_name'] = 'product-weight';
								} elseif ( 'Seat' === $attribute_name ) {
									$attribute['attribute_name'] = 'seat';
								} elseif ( 'Seat Depth' === $attribute_name ) {
									$attribute['attribute_name'] = 'seat-depth';
								} elseif ( 'Seat Height' === $attribute_name ) {
									$attribute['attribute_name'] = 'seat-height';
								}
		...
		function process_terms() {
			try {
				...
				foreach ( $this->posts as $post ) {
					if ( 'ptu_tax' == $post['post_type'] ) { // Post Types Unlimited Compaibility
						...
						$search_items               = array_key_exists( '_ptu_labels_search_items', $meta ) ? $meta['_ptu_labels_search_items'][0] : sprintf( _x( 'Search %s', 'taxonomy label', 'porto' ), $label );
						$all_items                  = array_key_exists( '_ptu_labels_all_items', $meta ) ? $meta['_ptu_labels_all_items'][0] : sprintf( _x( 'All %s', 'taxonomy label', 'porto' ), $label );
						$parent_item                = array_key_exists( '_ptu_labels_parent_item', $meta ) ? $meta['_ptu_labels_parent_item'][0] : sprintf( _x( 'Parent %s', 'taxonomy label', 'porto' ), $singular_name );
						$parent_item_colon          = array_key_exists( '_ptu_labels_parent_item_colon', $meta ) ? $meta['_ptu_labels_parent_item_colon'][0] : sprintf( _x( 'Parent %s:', 'taxonomy label', 'porto' ), $singular_name );
						$view_item                  = array_key_exists( '_ptu_labels_parent_view_item', $meta ) ? $meta['_ptu_labels_parent_view_item'][0] : sprintf( _x( 'View %s', 'taxonomy label', 'porto' ), $singular_name );
						$edit_item                  = array_key_exists( '_ptu_labels_parent_edit_item', $meta ) ? $meta['_ptu_labels_parent_edit_item'][0] : sprintf( _x( 'Edit %s', 'taxonomy label', 'porto' ), $singular_name );
						$update_item                = array_key_exists( '_ptu_labels_update_item', $meta ) ? $meta['_ptu_labels_update_item'][0] : sprintf( _x( 'Update %s', 'taxonomy label', 'porto' ), $singular_name );
						$add_new_item               = array_key_exists( '_ptu_labels_add_new_item', $meta ) ? $meta['_ptu_labels_add_new_item'][0] : sprintf( _x( 'Add New %s', 'taxonomy label', 'porto' ), $singular_name );
						$add_new_item_name          = array_key_exists( '_ptu_labels_add_new_item_name', $meta ) ? $meta['_ptu_labels_add_new_item_name'][0] : sprintf( _x( 'New %s', 'taxonomy label', 'porto' ), $singular_name );
						$popular_items              = array_key_exists( '_ptu_labels_popular_items', $meta ) ? $meta['_ptu_labels_popular_items'][0] : sprintf( _x( 'Popular %s', 'taxonomy label', 'porto' ), $label );
						$separate_items_with_commas = array_key_exists( '_ptu_labels_separate_items_with_commas', $meta ) ? $meta['_ptu_labels_separate_items_with_commas'][0] : sprintf( _x( 'Separate %s with commas', 'taxonomy label', 'porto' ), $label );
						$add_or_remove_items        = array_key_exists( '_ptu_labels_add_or_remove_items', $meta ) ? $meta['_ptu_labels_add_or_remove_items'][0] : sprintf( _x( 'Add or remove %s', 'taxonomy label', 'porto' ), $label );
						$choose_from_most_used      = array_key_exists( '_ptu_labels_choose_from_most_used', $meta ) ? $meta['_ptu_labels_choose_from_most_used'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'porto' ), $label );
						$choose_not_found           = array_key_exists( '_ptu_labels_choose_not_found', $meta ) ? $meta['_ptu_labels_choose_not_found'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'porto' ), $label );
						$not_found                  = array_key_exists( '_ptu_labels_not_found', $meta ) ? $meta['_ptu_labels_not_found'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'porto' ), $label );
						$back_to_items              = array_key_exists( '_ptu_labels_back_to_items', $meta ) ? $meta['_ptu_labels_back_to_items'][0] : sprintf( _x( 'Back to %s', 'taxonomy label', 'porto' ), $label );
					...
					} elseif ( 'acf-taxonomy' == $post['post_type'] ) { // ACF Compatibility
						global $acf_instances;
						if ( ! empty( $acf_instances ) ) {
							$taxonomy = maybe_unserialize( $post['post_content'] );
							$args     = $acf_instances['ACF_Taxonomy']->get_taxonomy_args( $taxonomy );
		...
		function process_posts( $index = 0 ) {
			...
			$allowed_import_types = array( 'post', 'product', 'portfolio', 'member', 'event', 'service', 'services', 'team', 'faq', 'practice' );
			...
			foreach ( $this->posts as $post ) {
				...
				} elseif ( 'acf-post-type' == $post['post_type'] ) {
					$whitelist[] = maybe_unserialize( $post['post_content'] )['post_type'];
				}
			}
			...
			if ( in_array( $post['post_type'], $allowed_import_types ) && isset( $_POST['one_import'] ) && 'true' == $_POST['one_import'] ) {
				$is_exists = new WP_Query( array( 'post_type' => $post['post_type'], 'post_status' => 'publish', 'posts_per_page' => 1, 'fields' => 'ids' ) );
				if ( $is_exists->have_posts() ) {
					unset( $this->posts );
					return;
				}
			}
			...
			if ( 'attachment' == $post['post_type'] && isset( $_POST['skip_media'] ) && 'true' == $_POST['skip_media'] ) {
				unset( $this->posts );
				return;
			}
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'page' == $post['post_type'] || 'block' == $post['post_type'] || 'porto_builder' == $post['post_type'] || 'member' == $post['post_type'] || 'portfolio' == $post['post_type'] || 'event' == $post['post_type'] || 'post' == $post['post_type'] || 'product' == $post['post_type'] || 'faq' == $post['post_type'] || 'service' == $post['post_type'] ) {
					...
					$meta_fields = array( '_wpb_post_custom_css', '_wpb_shortcodes_custom_css', '_elementor_data', '_elementor_edit_mode', 'vcv-pageContent', 'vcv-be-editor', '_elementor_page_assets', 'porto_elementor_page_assets_saved' );
		...
		function process_menu_item( $item ) {
			...
			$args = array(
				...
				'menu-item-popup_mobile_hide' => isset( $_menu_item_popup_mobile_hide ) ? $_menu_item_popup_mobile_hide : '',
