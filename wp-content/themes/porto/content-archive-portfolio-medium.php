...
<article <?php post_class( $post_class ); ?>>
	...
	<div class="row">
		...
		if ( $count ) :
			?>
		<div class="col-lg-4">
			<a aria-label="<?php esc_attr_e( 'Go to portfolio page', 'porto' ); ?>" class="portfolio-link" href="<?php
		...
		<?php endif; ?>
			...
			<?php if ( $ajax_attr || $portfolio_show_link_zoom ) : ?>
				<h4 aria-level="3" class="entry-title"><?php the_title(); ?></h4>
			<?php else : ?>
				<h4 aria-level="3" class="entry-title"><a href="<?php echo ! $show_external_link || ! $portfolio_link ? esc_url( get_the_permalink() ) : esc_url( $portfolio_link ); ?>"><?php the_title(); ?></a></h4>
			<?php endif; ?>
			...
			<?php endif; ?>
			<h4 class="sr-only"><?php esc_html_e( 'More Information', 'porto' ); ?></h4>
