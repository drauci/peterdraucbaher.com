...
<article <?php post_class( $post_class ); ?>>
	...
	<div class="portfolio-title<?php echo 'widewidth' === $porto_layout ? ' container m-t-lg' : ''; ?>">
		<div class="row">
			<div class="portfolio-nav-all col-lg-1">
				<a role="button" aria-label="<?php esc_attr_e( 'Back to list', 'porto' ); ?>" title="<?php esc_attr_e( 'Back to list', 'porto' ); ?>" data-bs-tooltip href="<?php echo get_post_type_archive_link( 'portfolio' ); ?>"><i class="fas fa-th"></i></a>
			</div>
			...
			<div class="portfolio-nav col-lg-1">
				<?php
				previous_post_link( '%link', '<div data-bs-tooltip title="' . esc_attr__( 'Previous', 'porto' ) . '" class="portfolio-nav-prev" aria-label="previous"><i class="fa"></i></div>' );
				next_post_link( '%link', '<div data-bs-tooltip title="' . esc_attr__( 'Next', 'porto' ) . '" class="portfolio-nav-next" aria-label="next"><i class="fa"></i></div>' );
				?>
			</div>
		</div>
	</div>
	...
	<div class="row<?php echo 'widewidth' === $porto_layout ? ' m-t-lg' : ''; ?>">
		...
		<div class="col-md-6">
						...
						<div class="portfolio-image<?php echo 1 == $image_count ? ' single' : ''; ?>">
							...
							if ( 'with-thumbs' == $slider_type ) :
								...
								<div class="thumb-gallery<?php echo ! $lightbox ? '' : ' lightbox'; ?>"<?php echo ! $lightbox ? '' : 'data-plugin-options="' . esc_attr( $lightbox_options ) . '"'; ?>>
									<div class="porto-carousel thumb-gallery-detail owl-carousel show-nav-hover has-ccols ccols-1" data-plugin-options="<?php echo esc_attr( $options ); ?>">
										<?php
										foreach ( $featured_images as $featured_image ) {
											...
											if ( $attachment ) {
												...
												<div class="thumb-gallery-item">
													...
													if ( $lightbox ) :
														?>
														<a aria-label="<?php esc_attr_e( 'Go to portfolio page', 'porto' ); ?>" href="<?php echo esc_url( $attachment['src'] ); ?>" title="<?php echo esc_attr( $attachment['alt'] ); ?>"><?php endif; ?>
								...
								if ( $image_count > 1 ) :
									...
									$grid_classes      = 'has-ccols ccols-' . $slider_thumbs_count;
									$options['margin'] = 10;
									$options['nav']    = false;
									$options['dots']   = false;
									$options['loop']   = false;
									if ( $slider_thumbs_count > 4 ) {
										$grid_classes  = 'has-ccols ccols-3 ccols-sm-4 ccols-xl-' . $slider_thumbs_count;
										$options['sm'] = 4;
										$options['xs'] = 3;
									}
									$options = json_encode( $options );
									wp_enqueue_script( 'porto-thumb-gallery' );
									?>
									<div class="porto-carousel thumb-gallery-thumbs owl-carousel show-nav-hover <?php echo esc_attr( $grid_classes ); ?>" data-plugin-options="<?php echo esc_attr( $options ); ?>">
										<?php
										foreach ( $featured_images as $featured_image ) {
			...
			if ( $porto_settings['share-enable'] && 'no' !== $share && ( 'yes' === $share || ( 'yes' !== $share && ! empty( $porto_settings['portfolio-share'] ) ) ) ) :
				...
				<div class="share-links-block">
					<h5 aria-level="3"><?php esc_html_e( 'Share', 'porto' ); ?></h5>
		...
		<?php endif; ?>
			...
			if ( get_the_content() ) :
				?>
				<h5 class="portfolio-desc" aria-level="3"><?php echo porto_strip_script_tags( __( 'Project <strong>Description</strong>', 'porto' ) ); ?></h5><?php endif; ?>
			...
			<?php if ( $portfolio_info ) : ?>
				<h5 class="m-t-sm" aria-level="3"><?php esc_html_e( 'More Information', 'porto' ); ?></h5>
				<div class="m-b-lg">
