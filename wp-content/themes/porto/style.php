...
/* search form */
#header .reveal-search-layout .searchform-fields { padding: 0 <?php echo (int) $b['grid-gutter-width'] / 2; ?>px; }
...
#header .reveal-search-layout .button-wrap { <?php echo porto_filter_output( $right ); ?>: <?php echo ( 20 + (int) $b['grid-gutter-width'] / 2 ); ?>px; }
#header .reveal-search-layout .btn-close-search-form { <?php echo ! $b['searchform-hover-color'] ? '' : 'color: ' . esc_html( $b['searchform-hover-color'] ); ?> }
#header .overlay-search-layout .selectric-cat,
#header .overlay-search-layout .text,
#header .overlay-search-layout .button-wrap { border-bottom: 2px solid <?php echo esc_html( $b['searchform-border-color'] ); ?>; }
#header .overlay-search-layout .btn-close-search-form { <?php echo ! $b['searchform-hover-color'] ? '' : 'color: ' . esc_html( $b['searchform-hover-color'] ); ?> }
...
<?php endif; ?>
<?php if ( ! empty( $b['sticky-searchform-popup-border-color'] ) ) : ?>
	@media (max-width: 991px) {
		#header.sticky-header .large-search-layout .searchform, #header.sticky-header .advanced-search-layout .searchform { border-color: <?php echo esc_html( $b['sticky-searchform-popup-border-color'] ); ?> !important; }
	}
	#header.sticky-header .large-search-layout .search-toggle:after, #header.sticky-header .advanced-search-layout .search-toggle:after { border-bottom-color: <?php echo esc_html( $b['sticky-searchform-popup-border-color'] ); ?> !important; }
	#header.sticky-header .large-search-layout .searchform { border-color: <?php echo esc_html( $b['sticky-searchform-popup-border-color'] ); ?> !important; }
<?php endif; ?>
<?php if ( isset( $porto_settings['search-live'] ) && $porto_settings['search-live'] ) : ?>
	...
	.live-search-list .autocomplete-suggestion .search-price { color: <?php echo esc_html( $porto_color_lib->isColorDark( $b['searchform-bg-color'] ) ? 'rgba(255, 255, 255, .7)' : $color_dark ); ?>; }
...
<?php if ( 10 == $header_type ) : ?>
	...
	@media (min-width: 992px) {
		...
		<?php if ( 'overlay' != $b['search-layout'] && 'reveal' != $b['search-layout'] ) : ?>
...
<?php elseif ( 19 == $header_type ) : ?>
	...
	#header.header-19 .searchform-popup .searchform { line-height: 36px; font-family: <?php echo sanitize_text_field( $b['body-font']['font-family'] ); ?>, sans-serif }
	#header .searchform input,
	#header .searchform.searchform-cats input { width: 200px; }
	<?php if ( ! empty( $b['search-border-radius'] ) ) : ?>
		#header.header-19 .search-rounded .searchform { border-radius: 4px; border: none; }
		#header .search-rounded .searchform .live-search-list { <?php echo porto_filter_output( $right ); ?>: 0; width: 100%; <?php echo porto_filter_output( $left ); ?>: auto; }
	<?php endif; ?>
	#header.header-19 .searchform input, #header.header-19 .searchform select, #header.header-19 .searchform .selectric .label, #header.header-19 .searchform button { height: 36px; }
...
elseif ( empty( $header_type ) && porto_header_type_is_side() ) :
	...
	if ( isset( $current_layout['side_header_pos'] ) ) {
		$side_position = $current_layout['side_header_pos'] ? 'right' : 'left';
	}
	...
	<?php endif; ?>
		...
		<?php if ( 'header_builder' == $porto_settings['header-type-select'] ) : ?>
			.header-builder > * { z-index: 1002; }
			.header-wrapper .header-builder { height: 100%; }
		<?php endif; ?>
		<?php
		...
		<?php if ( 'header_builder' == $porto_settings['header-type-select'] && ! empty( $porto_settings['side-menu-type'] ) ) : ?>
			.header-wrapper .header-builder { justify-content: center; }
		<?php endif; ?>
	...
	<?php if ( ! $side_mobile_visible ) : // default side layout ?>
		...
		<?php endif; ?>
			...
			.side-header-narrow-bar-top { height: auto; width: calc(100vw - var(--porto-scroll-w, 17px)); flex-direction: row; align-items: center; padding: 0 <?php echo esc_html( $b['grid-gutter-width'] / 2 ); ?>px; }
...
<?php if ( porto_header_type_is_side() && 'columns' == $b['side-menu-type'] ) : ?>
	...
	.header-side-nav .sidebar-menu .narrow ul.sub-menu:after { content: ''; position: absolute; bottom: calc( 100% - 1px); height: 100vh; width: calc(100% + 2px); left: -1px; background-color: inherit; border-width: 0 1px 0 1px; border-style: solid; border-color: inherit; }
	.header-side-nav .sidebar-menu .wide .popup > .inner:after,
...
@media (min-width: 992px) and (max-width: <?php echo (int) ( $container_xl + $b['grid-gutter-width'] - 1 ); ?>px) {
	.page-wrapper .device-hidden-tablet-extra {
		display: none;
	}
}
@media (min-width: <?php echo (int) ( $container_xl + $b['grid-gutter-width'] ); ?>px) and (max-width: <?php echo (int) $container_sl + (int) $b['grid-gutter-width'] * 2 - 1; ?>px) {
	.page-wrapper .device-hidden-desktop {
		display: none;
	}
}
@media (min-width: <?php echo (int) $container_sl + (int) $b['grid-gutter-width'] * 2; ?>px) {
...
<?php if ( $legacy_mode && class_exists( 'Woocommerce' ) ) : ?>
...
<?php if ( $b['thumb-padding'] ) : ?>
	ul.mega-menu li.menu-item > a > .thumb-info-preview .thumb-info-wrapper,
	ul.sidebar-menu li.menu-item > a > .thumb-info-preview .thumb-info-wrapper,
	...
	div.thumb-info, span.thumb-info { background-color: <?php echo porto_if_light( '#fff', $color_dark_3 ); ?>; border-color: <?php echo porto_if_light( '#ddd', $color_dark_3 ); ?>; }
	span.thumb-info-social-icons { border-top: <?php echo porto_if_light( '1px dotted #ddd', '1px solid ' . $porto_color_lib->lighten( $dark_bg, 12 ) ); ?>; }
	...
	<?php if ( class_exists( 'Woocommerce' ) ) : ?>
		...
		.product-image-slider.owl-carousel:not(.has-ccols-spacing) .img-thumbnail { width: 99.8%; padding: 3px; }
		.product-image-slider { --porto-el-spacing: 0; }
		.product-image-slider>.img-thumbnail { border-width: 0; }
		.product-image-slider.has-ccols-spacing>.img-thumbnail, .product-image-slider.has-ccols-spacing>.vd-image { padding-right: calc(var(--porto-el-spacing, 2px) / 2); padding-left: calc( var(--porto-el-spacing, 2px ) / 2); }
...
<?php else : ?>
	.page-wrapper .fdm-item-image,
	div.thumb-info, span.thumb-info, article.thumb-info { border-width: 0; background: none; }
	.thumb-info-caption .thumb-info-caption-text { padding: 15px 0; margin-bottom: 0; }
	span.thumb-info-social-icons { padding: 0; }
...
<?php if ( $dark ) : ?>
	...
	.select2-container--default .select2-selection--single, .select2-dropdown { background-color: var(--porto-normal-bg); }
<?php else : ?>
...
<?php if ( ! empty( $b['sidebar-pd'] ) && ( ! empty( $b['sidebar-pd']['padding-top'] ) || ! empty( $b['sidebar-pd']['padding-right'] ) || ! empty( $b['sidebar-pd']['padding-bottom'] ) || ! empty( $b['sidebar-pd']['padding-left'] ) ) ) : ?>
	.sidebar-content, .woocommerce-page.archive .sidebar-content aside.widget {
		<?php if ( ! empty ( $b['sidebar-pd']['padding-top'] ) ) : ?>
			padding-top: <?php echo porto_config_value( $b['sidebar-pd']['padding-top'], 'px' ); ?>;
		<?php endif; ?>
		<?php if ( ! empty ( $b['sidebar-pd']['padding-right'] ) ) : ?>
			padding-right: <?php echo porto_config_value( $b['sidebar-pd']['padding-right'], 'px' ); ?>;
		<?php endif; ?>
		<?php if ( ! empty ( $b['sidebar-pd']['padding-bottom'] ) ) : ?>
			padding-bottom: <?php echo porto_config_value( $b['sidebar-pd']['padding-bottom'], 'px' ); ?>;
		<?php endif; ?>
		<?php if ( ! empty ( $b['sidebar-pd']['padding-left'] ) ) : ?>
			padding-left: <?php echo porto_config_value( $b['sidebar-pd']['padding-left'], 'px' ); ?>;
		<?php endif; ?>
	}
...
<?php if ( 17 == $header_type ) : ?>
	...
	<?php endif; ?>
	#header .searchform-popup .searchform .live-search-list { left: 0; right: 0; }
	@media (min-width: 768px) {
		#header .header-main .header-left,
		#header .header-main .header-center,
...
<?php endif; ?>
.header-wrapper .separator { border-left-color: <?php echo porto_filter_output( $porto_color_lib->isColorDark( isset( $b['header-link-color']['regular'] ) ? $b['header-link-color']['regular'] : '' ) ? 'rgba(0, 0, 0, .04)' : 'rgba(255, 255, 255, .09)' ); ?> }
.header-top .separator { border-left-color: <?php echo porto_filter_output( $porto_color_lib->isColorDark( isset( $b['header-top-link-color']['regular'] ) ? $b['header-top-link-color']['regular'] : '' ) ? 'rgba(0, 0, 0, .04)' : 'rgba(255, 255, 255, .09)' ); ?> }
...
.fixed-header #header.sticky-header .overlay-search-layout .searchform {
	background: <?php echo esc_html( 'rgba(' . $porto_color_lib->hexToRGB( $b['searchform-bg-color'] ) . ', .95)' ); ?>;
}
<?php endif; ?>
...
<?php if ( $legacy_mode && 'header_builder_p' != $b['header-type-select'] && ! empty( $b['mainmenu-wrap-padding-sticky'] ) ) : ?>
...
} elseif ( isset( $b['mainmenu-popup-border'] ) && $b['mainmenu-popup-border'] ) {
...
<?php if ( $b['searchform-popup-border-color'] ) : ?>
	#header .large-search-layout .search-toggle:after, #header .advanced-search-layout .search-toggle:after { border-bottom-color: <?php echo esc_html( $b['searchform-popup-border-color'] ); ?>; }
	#header .large-search-layout .searchform { border-color: <?php echo esc_html( $b['searchform-popup-border-color'] ); ?>; }
	@media (max-width: 991px) {
		#header .large-search-layout .searchform, #header .advanced-search-layout .searchform { border-color: <?php echo esc_html( $b['searchform-popup-border-color'] ); ?>; }
	}
<?php endif; ?>
<?php if ( $b['searchform-bg-color'] ) : ?>
	#header .simple-search-layout .search-toggle:after { border-bottom-color: <?php echo esc_html( $b['searchform-bg-color'] ); ?> }
<?php endif; ?>
...
#header .mobile-toggle {
	...
	<?php if ( isset( $b['mobile-menu-toggle-bg-color'] ) && 'transparent' == $b['mobile-menu-toggle-bg-color'] ) : ?>
		font-size: 20px;
...
<?php if ( $legacy_mode && ( ! isset( $porto_settings['show-sticky-minicart'] ) || ! $porto_settings['show-sticky-minicart'] ) ) : ?>
	...
	.sticky-header [data-widget_type="porto_hb_mini_cart.default"] { display: none !important; }
<?php endif; ?>
...
<?php elseif ( 11 == $header_type && $b['header-top-border']['border-top'] && '0px' != $b['header-top-border']['border-top'] ) : ?>
	...
	@media (min-width: 992px) {
		...
		#header .main-menu > li.menu-item:hover > a {
			border-top: <?php echo esc_html( $b['header-top-border']['border-top'] ); ?> solid <?php echo 'transparent' == $b['mainmenu-toplevel-hbg-color'] ? ( isset( $b['mainmenu-toplevel-link-color']['hover'] ) ? $b['mainmenu-toplevel-link-color']['hover'] : ( $main_menu_level1_abg_color ? 'var(--porto-primary-color-inverse)' : '' ) ) : $b['mainmenu-toplevel-hbg-color']; ?>;
		}
	}
<?php elseif ( 18 == $header_type ) : ?>
...
<?php if ( empty( $b['mobile-panel-type'] ) ) : ?>
	...
	.page-wrapper { transition: <?php echo $rtl ? 'right': 'left'; ?> .3s }
...
<?php elseif ( 'side' == $b['mobile-panel-type'] ) : ?>
	...
	?>
	.page-wrapper,
	#header.sticky-header .header-main.sticky { transition: <?php echo $rtl ? 'right' : 'left'; ?> .3s }
	<?php if ( $rtl) : ?>
	html.panel-opened .page-wrapper,
	html.panel-opened #header.sticky-header .header-main.sticky { right: <?php echo 'left' == $mobile_panel_pos ? '-260px' : '260px'; ?>; }
	<?php else : ?>
		html.panel-opened .page-wrapper,
...
<?php if ( isset( $b['breadcrumbs-path-margin'] ) && $b['breadcrumbs-path-margin'] ) : ?>
	.page-top .breadcrumbs-wrap {
		<?php if ( isset( $b['breadcrumbs-path-margin']['margin-top'] ) && 0 !== $b['breadcrumbs-path-margin']['margin-top'] ) : ?>
			margin-top: <?php echo porto_config_value( $b['breadcrumbs-path-margin']['margin-top'] ); ?>px;
		<?php endif; ?>
		<?php if ( isset( $b['breadcrumbs-path-margin']['margin-' . $right] ) && 0 !== $b['breadcrumbs-path-margin']['margin-' . $right] ) : ?>
			margin-right: <?php echo porto_config_value( $b['breadcrumbs-path-margin']['margin-' . $right] ); ?>px;
		<?php endif; ?>
		<?php if ( isset( $b['breadcrumbs-path-margin']['margin-bottom'] ) && 0 !== $b['breadcrumbs-path-margin']['margin-bottom'] ) : ?>
			margin-bottom: <?php echo porto_config_value( $b['breadcrumbs-path-margin']['margin-bottom'] ); ?>px;
		<?php endif; ?>
		<?php if ( isset( $b['breadcrumbs-path-margin']['margin-' . $left] ) && 0 !== isset( $b['breadcrumbs-path-margin']['margin-' . $left] ) ) : ?>
			margin-left: <?php echo porto_config_value( $b['breadcrumbs-path-margin']['margin-' . $left] ); ?>px;
		<?php endif; ?>
	}
<?php endif; ?>
...
<?php if ( class_exists( 'Woocommerce' ) ) : ?>
	...
	<?php endif; ?>
	@media (min-width: <?php echo (int) ( $container_xl + $b['grid-gutter-width'] ); ?>px) {
		.divider-line.ccols-xl-6 > .product-col:nth-child(6n),
		.divider-line.ccols-xl-5 > .product-col:nth-child(5n),
		.divider-line.ccols-xl-4 > .product-col:nth-child(4n),
		.divider-line.ccols-xl-3 > .product-col:nth-child(3n),
		.divider-line.ccols-xl-2 > .product-col:nth-child(2n) { border-right-width: 0; }
	}
	@media (min-width: 768px) and (max-width: <?php echo ( (int) $container_xl + (int) $b['grid-gutter-width'] - 1 ); ?>px) {
		.divider-line.ccols-lg-5 > .product-col:nth-child(5n),
		.divider-line.ccols-lg-4 > .product-col:nth-child(4n),
		.divider-line.ccols-lg-3 > .product-col:nth-child(3n),
		.divider-line.ccols-lg-2 > .product-col:nth-child(2n) { border-right-width: 0; }
	}
...
<?php if ( defined( 'ELEMENTOR_VERSION' ) ) : ?>
	...
	<?php if ( defined( 'ELEMENTOR_PRO_VERSION' ) ) : ?>
		...
		.elementor-products-grid nav.woocommerce-pagination { margin-top: 0 !important; }
		.elementor-widget-wc-archive-products #infscr-loading { top: calc(100% + 20px); }
		@media (min-width: 576px) {
...
<?php if ( ! empty( $porto_settings['show-icon-menus-mobile'] ) ) : ?>
	...
	.porto-sticky-navbar .label { display: block;<?php echo ( isset( $b['h4-font']['font-family'] ) ? ' font-family:' . sanitize_text_field( $b['h4-font']['font-family'] ) . ', sans-serif; ' : '' ); ?>text-transform: uppercase; font-size: 9px; letter-spacing: -.5px; font-weight: 600; }
	...
	@media (max-width: 575px) {
		...
		body {
			--porto-icon-menus-mobile: 60px;
		}
	}
<?php endif; ?>

/*---------------- Sidebar Width ---------------------*/
<?php if ( ! empty( $b['global-sidebar-width'] ) ) : ?>
	@media( min-width: 992px ) {
		.sidebar {
			width: <?php echo esc_attr( $b['global-sidebar-width'] ); ?>
		}
		.main-content.col-lg-9 {
			width: calc( 100% - <?php echo esc_attr( $b['global-sidebar-width'] ); ?> );
		}
		.main-content.col-lg-6 {
			width: calc( 100% - 2 * <?php echo esc_attr( $b['global-sidebar-width'] ); ?> );
		}
	}
<?php endif; ?>
<?php if ( ! empty( $b['woo-sidebar-width'] ) ) : ?>
	@media( min-width: 992px ) {
		.woocommerce.archive .sidebar,
		.single-product .sidebar {
			width: <?php echo esc_attr( $b['woo-sidebar-width'] ); ?>
		}
		.woocommerce.archive .main-content.col-lg-9,
		.single-product .main-content.col-lg-9 {
			width: calc( 100% - <?php echo esc_attr( $b['woo-sidebar-width'] ); ?> );
		}
		.woocommerce.archive .main-content.col-lg-6,
		.single-product .main-content.col-lg-6 {
			width: calc( 100% - 2 * <?php echo esc_attr( $b['woo-sidebar-width'] ); ?> );
		}
	}
<?php endif; ?>