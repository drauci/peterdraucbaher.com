 ...
 */
$el_class        = $full_height = $min_height = $parallax_speed_bg = $parallax_speed_video = $full_width = $equal_height = $flex_row = $columns_placement = $content_placement = $parallax = $parallax_image = $css = $el_id = $video_bg = $video_bg_url = $video_bg_parallax = $css_animation = $no_padding = $conditional_render = '';
$disable_element = '';
...
if ( $is_section && $show_divider ) {
	...
	if ( $divider_icon_color ) {
		$dv_arr[] = $divider_icon_color;
	}
	if ( $divider_icon_bg_color ) {
		$dv_arr[] = $divider_icon_bg_color;
	}
	if ( $divider_icon_border_color ) {
		$dv_arr[] = $divider_icon_border_color;
	}
	if ( $divider_icon_wrap_border_color ) {
		$dv_arr[] = $divider_icon_wrap_border_color;
	}
	$divider_class = 'divider' . hash( 'md5',  json_encode( $dv_arr ) );
...
if ( $has_video_bg || ( 'mp4' === $video_type ) ) {
	...
	if ( 'mp4' === $video_type && empty( $parallax ) ) {
		...
		wp_enqueue_script( 'porto-video-bg' );
...
if ( ! empty( $atts['hover_split'] ) ) {
	wp_enqueue_script( 'porto-hover-split' );
	porto_enqueue_link_style( 'porto-hv-split', PORTO_CSS . '/part/hv-split.css' );
...
if ( isset( $porto_settings_optimize['lazyload'] ) && $porto_settings_optimize['lazyload'] && ! vc_is_inline() ) {
	...
	if ( ! empty( $matches[2] ) || ! empty( $matches[3] ) ) {
		...
		if ( ! empty( $porto_settings_optimize['preload_custom'] ) ) {
			...
			if ( ! empty( $font_urls ) ) {
				$font_urls = array_map( 'trim', $font_urls );
			}
			$preloads  = array_merge( $preloads, $font_urls );
		}
...
if ( class_exists( 'WPBMap' ) ) {
	...
	if ( ! empty( $sc['params'] ) && class_exists( 'PortoShortcodesClass' ) && method_exists( 'PortoShortcodesClass', 'get_global_hashcode' ) ) {
		foreach ( $original_atts as $key => $item ) {
			if ( in_array( $key, array( 'ph_space_sticky','pv_space_sticky' ) ) ) {
				$original_atts[ $key ] = str_replace( '"', '``', $item );
			}
		}
		$shortcodeClass = 'wpb_custom_' . PortoShortcodesClass::get_global_hashcode( $original_atts, 'vc_row', $sc['params'] );
		$css_classes[] = $shortcodeClass;
		$internal_css = PortoShortcodesClass::generate_wpb_css( 'vc_row', $original_atts );

		if ( isset( $atts['sticky_gradient'] ) && 'yes' == $atts['sticky_gradient'] ) {
			$internal_css .= '#header.sticky-header .' . $shortcodeClass . '.header-main, .fixed-header #header.sticky-header .' . $shortcodeClass . '{ background: linear-gradient(' . ( $atts['sticky_angle'] ? $atts['sticky_angle'] : 0 ) . 'deg , ' . $atts['sticky_top_bg'] . ' ' . ( $atts['sticky_top_loc'] ? $atts['sticky_top_loc'] : 0 ) . '%, ' .  $atts['sticky_bottom_bg'] . ' ' . ( $atts['sticky_bottom_loc'] ? $atts['sticky_bottom_loc'] : 100 ) . '%) !important;}';
		} elseif ( ! empty( $atts['sticky_bg'] ) ) {
			$internal_css .= '#header.sticky-header .' . $shortcodeClass . '.header-main, .fixed-header #header.sticky-header .' . $shortcodeClass . '{ background-color: ' . esc_html( $atts['sticky_bg'] ) . ' !important; }';
		}
	}
}
