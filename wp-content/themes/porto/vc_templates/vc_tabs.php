...
$tabs_nav .= '<ul role="tablist" class="nav nav-tabs' . $ul_class . ( 'tabs-simple' == $type ? ' featured-boxes ' . esc_attr( $icon_style ) : '' ) . '">';
foreach ( $tabs as $tab ) {
	...
	$tab_title               = str_replace( '`}`', ']', $tab_title );
	$tab_title               = do_shortcode( $tab_title );
	...
	if ( $tab_title ) {
		...
		$tabs_nav .= '<li role="presentation" class="nav-item"><a role="tab" href="#' . esc_attr( $tab_id ) . '" id="' . esc_attr( $tab_id ) . '-title" class="nav-link" data-bs-toggle="tab">';
		$tab_id   .= '-title';
...
$tabs_nav .= '</ul>';
$tabs_nav  = preg_replace( '/<li role="presentation" class="nav-item">/', '<li role="presentation" class="nav-item active">', $tabs_nav, 1 );
...
if ( 'custom' == $skin && $color ) {
	...
	if ( $porto_settings['body-font']['color'] ) {
		$tb_clr[] = $porto_settings['body-font']['color'];
	}

	$tabs_class = 'tabs' . hash( 'md5',  json_encode( $tb_clr ) );
