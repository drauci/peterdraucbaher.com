...
if ( $social_share ) {
	...
	if ( $share_facebook || $share_twitter || $share_linkedin || $share_googleplus || $share_pinterest || $share_email || $share_vk || $share_xing || $share_tumblr || $share_reddit || $share_vimeo || $share_instagram || $share_whatsapp ) :
		...
		if ( $share_phone ) :
			$share_links .= '<div data-bs-tooltip data-bs-placement="bottom" title="' . esc_attr__( 'Phone', 'porto' ) . '" class="share-phone"><i class="porto-icon-phone-call"></i>' . esc_html( $share_phone ) . '</div>';
				endif;
			$share_links .= '</div>';
		endif;
}
?>
	...
	<div class="member-overview row">
									...
									<div class="img-thumbnail">
										...
										<?php if ( ! empty( $porto_settings['member-zoom'] ) ) : ?>
											...
											if ( ! is_singular( 'member' ) ) :
												?>
												<a aria-label="<?php esc_attr_e( 'Go to member page', 'porto' ); ?>" class="link" href="<?php the_permalink(); ?>"><i class="fas fa-link"></i></a><?php endif; ?>
										<?php endif; ?>
									</div>
								</div>
								<?php
		...
		<?php endif; ?>
			...
			<?php echo ! $role ? '' : '<h4 aria-level="3" class="member-role">' . esc_html( $role ) . '</h4>'; ?>
			...
			<?php if ( $social_share && isset( $porto_settings['member-socials-pos'] ) && '' == $porto_settings['member-socials-pos'] && isset( $social_links_adv_pos ) && ! $social_links_adv_pos ) : ?>
				<div class="col-lg-6 share-links-block<?php echo ! $member_link ? '' : ' d-flex justify-content-lg-end'; ?>">
					<h5 aria-level="4"><?php esc_html_e( 'Follow Me', 'porto' ); ?></h5>
					<?php echo porto_filter_output( $share_links ); ?>
				</div>
			<?php endif; ?>
			</div>
