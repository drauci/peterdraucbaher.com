...
if ( $count ) :
	...
	if ( $portfolio_columns > 2 && $attachment ) {
		if ( $attachment['width'] > $attachment['height'] * abs( isset( $porto_settings['portfolio-archive-masonry-ratio'] ) ? $porto_settings['portfolio-archive-masonry-ratio'] : '2.4' ) ) {
			$post_class[] = ' w2';
			if ( ! isset( $image_size ) ) {
	...
	$portfolio_slider_ids_arr = explode( ',', ! empty( $porto_portfolio_slider ) ? $porto_portfolio_slider : '' );
	...
	<article <?php post_class( $post_class ); ?>>
		...
		<div class="portfolio-item <?php echo esc_attr( $portfolio_view ); ?>">
			...
			<?php endif; ?>
			<a aria-label="<?php esc_attr_e( 'Go to portfolio page', 'porto' ); ?>" class="text-decoration-none portfolio-link" href="<?php
			if ( $portfolio_show_link_zoom ) {
				foreach ( $featured_images as $featured_image ) {
					...
					if ( $attachment ) {
						...
						break;
					} else if ( -1 == $attachment_id ) {
						echo esc_url( $featured_image['src'] );
						break;
					}
				}
			} else {
				...
				<span class="thumb-info <?php echo esc_attr( $class ); ?>">
					<span class="thumb-info-wrapper">
						...
						<?php
							...
							if ( $show_info && ! empty( $porto_settings['portfolio-archive-readmore'] ) ) : ?>
							<span class="thumb-info-title">
			...
			<?php if ( 'outimage' == $portfolio_view ) : ?>
				<?php if ( $portfolio_columns > 4 ) : ?>
					<h5 aria-level="3" class="portfolio-title m-t-md m-b-none"><?php the_title(); ?></h5>
				<?php else : ?>
					<h4 aria-level="3" class="portfolio-title m-t-md m-b-none"><?php the_title(); ?></h4>
				<?php endif; ?>
				<?php if ( $sub_title ) : ?>
